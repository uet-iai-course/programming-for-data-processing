...
| 27 | 14/09/2026 | **Đề BTL (`projects/project_airbnb.html`) bản sửa lớn**: bỏ thành phố đối chứng; chấm bằng **bộ test chung** chạy trên repo mọi nhóm, kể cả bước LLM (khoá API GV qua biến môi trường); 3 mốc có ngày: lập nhóm 20/09/2026, `git tag final` 22/11/2026, vấn đáp theo lịch thi Trường; bỏ mốc proposal; vấn đáp không có phần trình bày (bỏ slides.pdf); rubric "công bố dần, chưa chốt" (bản chi tiết ở `project_airbnb_rubric-draft.html`, draft-only); rút gọn danh sách tệp, mục 2–6, phần LLM, môi trường; bảng 26 thành phố thu gọn `<details>`; bỏ "Lỗi thường gặp". Kèm sửa tay GV (bỏ mô tả run_pipeline, bỏ danh sách sản phẩm bàn giao). **Chưa publish**: index (Vấn đáp BTL + lộ trình), deck 1/10/15, lab-10/13/14. |
| 28 | 14/09/2026 | Publish nốt cụm đồng bộ với đề BTL mới: `index.html` (hàng "Vấn đáp BTL", lộ trình bỏ proposal, "lập nhóm 20/09" + "final 22/11"), deck 1 (slide BTL: bộ test chung, bỏ đối chứng), deck 10 (held-out → bộ test chung), deck 15 ("Vấn đáp bài tập lớn", bỏ trình bày), lab-10/13/14 (checklist). 7 file. |
...
| 29 | 11/09/2026 | Đổi tên Bài 3 trong `2627-1/index.html` thành **NumPy và tư duy vector hoá**, khớp deck. Chỉ sửa tên liên kết; main `2e0bb57`. URL index public trả HTTP 200 và có tiêu đề mới. |
